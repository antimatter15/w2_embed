...
function blip_render(blipid, parent){ //a wrapper around renderBlip that adds chrome related things
  ...
  for(var i in blipscope){
    blipscope[i] = prefix+i+'('+(bliparr.length -1)+');return false';
  }
...
window[globname][embed_id] = {
  ...
  reply: function(blipcount){
    var blipid = bliparr[blipcount], blip = msg.data.blips[blipid];
...
function render_header(){
  var doc = config.header;
  doc.innerHTML = template_renderer(header_template, tpl_environ)

  Map(CName(doc, 'participantlist'), function(el){
...
function render_footer(){
  var doc = config.footer;
  doc.innerHTML = template_renderer(footer_template, tpl_environ)
